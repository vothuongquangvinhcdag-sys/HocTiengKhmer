/* =========================================================
   VOCABULARY DATA
   HỌC TIẾNG KHMER
========================================================= */

/* =========================================================
   CHỦ ĐỀ 1: SỐ ĐẾM
========================================================= */

/* =========================================================
   CHỦ ĐỀ 1: SỐ ĐẾM
   SẮP XẾP TỪ NHỎ ĐẾN LỚN
========================================================= */

const numbers = [
  /* -------------------------------------------------------
     SỐ ĐẾM CƠ BẢN
  ------------------------------------------------------- */

  {
    khmer: "លេខ",
    roman: "Lêk",
    vietnamese: "Số đếm",
    image: "🔢",
  },

  {
    khmer: "មួយ",
    roman: "Muôi",
    vietnamese: "Viết bằng số khmer: ១",
    image: "1",
  },

  {
    khmer: "ពីរ",
    roman: "Pi",
    vietnamese: "Viết bằng số khmer: ២",
    image: "2",
  },

  {
    khmer: "បី",
    roman: "Bây",
    vietnamese: "Viết bằng số khmer: ៣",
    image: "3",
  },

  {
    khmer: "បួន",
    roman: "Buôn",
    vietnamese: "Viết bằng số khmer: ៤",
    image: "4",
  },

  {
    khmer: "ប្រាំ",
    roman: "Prăm",
    vietnamese: "Viết bằng số khmer: ៥",
    image: "5",
  },

  {
    khmer: "ប្រាំមួយ",
    roman: "Prăm - Muôi",
    vietnamese: "Viết bằng số khmer: ៦",
    image: "6",
  },

  {
    khmer: "ប្រាំពីរ",
    roman: "Prăm - Pi",
    vietnamese: "Viết bằng số khmer: ៧",
    image: "7",
  },

  {
    khmer: "ប្រាំបី",
    roman: "Prăm - Bây",
    vietnamese: "Viết bằng số khmer: ៨",
    image: "8",
  },

  {
    khmer: "ប្រាំបួន",
    roman: "Prăm - Buôn",
    vietnamese: "Viết bằng số khmer: ៩",
    image: "9",
  },

  {
    khmer: "ដប់",
    roman: "Đóp",
    vietnamese: "Viết bằng số khmer: ១០",
    image: "10",
  },

  {
    khmer: "ដប់មួយ",
    roman: "Đóp - Muôi",
    vietnamese: "Viết bằng số khmer: ១១",
    image: "11",
  },

  {
    khmer: "ដប់ពីរ",
    roman: "Đóp - Pi",
    vietnamese: "Viết bằng số khmer: ១២",
    image: "12",
  },

  {
    khmer: "ដប់បី",
    roman: "Đóp - Bây",
    vietnamese: "Viết bằng số khmer: ១៣",
    image: "13",
  },

  {
    khmer: "ដប់បួន",
    roman: "Đóp - Buôn",
    vietnamese: "Viết bằng số khmer: ១៤",
    image: "14",
  },

  {
    khmer: "ដប់ប្រាំ",
    roman: "Đóp - Prăm",
    vietnamese: "Viết bằng số khmer: ១៥",
    image: "15",
  },

  {
    khmer: "ដប់ប្រាំមួយ",
    roman: "Đóp - Prăm - Muôi",
    vietnamese: "Viết bằng số khmer: ១៦",
    image: "16",
  },

  {
    khmer: "ដប់ប្រាំពីរ",
    roman: "Đóp - Prăm - Pi",
    vietnamese: "Viết bằng số khmer: ១៧",
    image: "17",
  },

  {
    khmer: "ដប់ប្រាំបី",
    roman: "Đóp - Prăm - Bây",
    vietnamese: "Viết bằng số khmer: ១៨",
    image: "18",
  },

  {
    khmer: "ដប់ប្រាំបួន",
    roman: "Đóp - Prăm - Buôn",
    vietnamese: "Viết bằng số khmer: ១៩",
    image: "19",
  },

  {
    khmer: "ម្ភៃ",
    roman: "Mô-phây",
    vietnamese: "Viết bằng số khmer: ២០",
    image: "20",
  },

  /* -------------------------------------------------------
     HÀNG CHỤC
  ------------------------------------------------------- */

  {
    khmer: "សាមសិប",
    roman: "Sam (sâp)",
    vietnamese: "Viết bằng số khmer: ៣០",
    image: "30",
    note: "Có thể đọc ngắn khi ghép với hàng đơn vị.",
  },

  {
    khmer: "សែសិប",
    roman: "Se (sâp)",
    vietnamese: "Viết bằng số khmer: ៤០",
    image: "40",
  },

  {
    khmer: "ហាសិប",
    roman: "Ha (sâp)",
    vietnamese: "Viết bằng số khmer: ៥០",
    image: "50",
  },

  {
    khmer: "ហុកសិប",
    roman: "Hốc (sâp)",
    vietnamese: "Viết bằng số khmer: ៦០",
    image: "60",
  },

  {
    khmer: "ចិតសិប",
    roman: "Chet (sâp)",
    vietnamese: "Viết bằng số khmer: ៧០",
    image: "70",
  },

  {
    khmer: "ប៉ែតសិប",
    roman: "Pet (sâp)",
    vietnamese: "Viết bằng số khmer: ៨០",
    image: "80",
  },

  {
    khmer: "កៅសិប",
    roman: "Cau / Kao (sâp)",
    vietnamese: "Viết bằng số khmer: ៩០",
    image: "90",
  },

  /* -------------------------------------------------------
     HÀNG TRĂM
  ------------------------------------------------------- */

  {
    khmer: "មួយរយ",
    roman: "Muôi-rôi",
    vietnamese: "Viết bằng số khmer: ១០០",
    image: "100",
  },

  /* -------------------------------------------------------
     HÀNG NGHÌN
  ------------------------------------------------------- */

  {
    khmer: "មួយពាន់",
    roman: "Muôi-pon",
    vietnamese: "Viết bằng số khmer: ១០០០",
    image: "1 nghìn",
  },

  {
    khmer: "មួយម៉ឺន",
    roman: "Muôi-mơn",
    vietnamese: "Viết bằng số khmer: ១០០០០",
    image: "10 nghìn",
  },

  {
    khmer: "ពីរម៉ឺន",
    roman: "Pi mơn",
    vietnamese: "Viết bằng số khmer: ២០០០០",
    image: "20 nghìn",
  },

  {
    khmer: "បីម៉ឺនប្រាំពាន់",
    roman: "Bây mơn prăm pon",
    vietnamese: "Viết bằng số khmer: ៣៥០០០",
    image: "35 nghìn",
  },

  {
    khmer: "បួនម៉ឺនប្រាំពាន់",
    roman: "Buôn mơn prăm pon",
    vietnamese: "Viết bằng số khmer: ៤៥០០០",
    image: "45 nghìn",
  },

  {
    khmer: "ប្រាំម៉ឺន",
    roman: "Prăm mơn",
    vietnamese: "Viết bằng số khmer: ៥០០០០",
    image: "50 nghìn",
  },

  {
    khmer: "ប្រាំបួនម៉ឺន",
    roman: "Prăm-buôn mơn",
    vietnamese: "Viết bằng số khmer: ៩០០០០",
    image: "90 nghìn",
  },

  {
    khmer: "ដប់ម៉ឺន",
    roman: "Đóp mơn",
    vietnamese: "Viết bằng số khmer: ១០០០០០",
    image: "100 nghìn",
  },

  {
    khmer: "ម្ភៃម៉ឺន",
    roman: "Mô-phây mơn",
    vietnamese: "Viết bằng số khmer: ២០០០០០",
    image: "200 nghìn",
  },

  {
    khmer: "ម្ភៃម៉ឺនប្រាំពាន់",
    roman: "Mô-phây mơn prăm pon",
    vietnamese: "Viết bằng số khmer: ២០៥០០០",
    image: "205 nghìn",
  },

  /* -------------------------------------------------------
     HÀNG TRIỆU
  ------------------------------------------------------- */

  {
    khmer: "មួយលាន",
    roman: "Muôi-lian",
    vietnamese: "Viết bằng số khmer: ១០០០០០០",
    image: "1 triệu",
  },

  {
    khmer: "មួយលានម្ភៃម៉ឺន",
    roman: "Muôi-lian mô-phây mơn",
    vietnamese: "Viết bằng số khmer: ១២០០០០០",
    image: "1,2 triệu",
  },

  {
    khmer: "ប្រាំលានហាសិបម៉ឺន",
    roman: "Prăm-lian ha-sâp prăm mơn",
    vietnamese: "Viết bằng số khmer: ៥៥០០០០០",
    image: "5,5 triệu",
  },

  {
    khmer: "មួយកោដិ",
    roman: "Muôi-coth",
    vietnamese: "Viết bằng số khmer: ១០០០០០០០",
    image: "10 triệu",
  },

  {
    khmer: "ម្ភៃលានប្រាំម៉ឺន",
    roman: "Mô-phây lian prăm mơn",
    vietnamese: "Viết bằng số khmer: ២០០៥០០០០",
    image: "20,05 triệu",
  },

  {
    khmer: "មួយរយលាន",
    roman: "Muôi-rôi lian",
    vietnamese: "Viết bằng số khmer: ១០០០០០០០០",
    image: "100 triệu",
  },

  /* -------------------------------------------------------
     HÀNG TỶ
  ------------------------------------------------------- */

  {
    khmer: "មួយពាន់លាន",
    roman: "Muôi-pon lian",
    vietnamese: "Viết bằng số khmer: ១០០០០០០០០០",
    image: "1 tỷ",
  },

  {
    khmer: "មួយពាន់លានប្រាំរយលាន",
    roman: "Muôi pon prăm rôi lian",
    vietnamese: "Viết bằng số khmer: ១៥០០០០០០០០",
    image: "1,5 tỷ",
  },

  /* -------------------------------------------------------
     HÀNG NGHÌN TỶ
  ------------------------------------------------------- */

  {
    khmer: "មួយទ្រីលាន",
    roman: "Muôi tri-lian",
    vietnamese: "Viết bằng số khmer: ១០០០០០០០០០០០០",
    image: "1.000 tỷ",
  },

  {
    khmer: "ពីរទ្រីលាន",
    roman: "Pi tri-lian",
    vietnamese: "Viết bằng số khmer: ២០០០០០០០០០០០០",
    image: "2.000 tỷ",
  },

  {
    khmer: "បីទ្រីលាន",
    roman: "Bây tri-lian",
    vietnamese: "Viết bằng số khmer: ៣០០០០០០០០០០០០",
    image: "3.000 tỷ",
  },
];

/* =========================================================
   CHỦ ĐỀ 2: TỪ XƯNG HÔ GIAO TIẾP HÀNG NGÀY & BẠN BÈ
========================================================= */

const pronouns = [
    {
    khmer: "ខ្ញុំ",
    roman: "Kh'nhôm",
    vietnamese: "Tôi",
    image: "🙋",
  },

  {
    khmer: "យើងខ្ញុំ",
    roman: "Dơng kh'nhôm",
    vietnamese: "Chúng tôi",
    image: "👥",
  },

  {
    khmer: "យើង",
    roman: "Dơng",
    vietnamese: "Ta / Chúng ta",
    image: "👥",
  },

  {
    khmer: "យើង",
    roman: "Dơng",
    vietnamese: "Mình",
    image: "👫",
  },

  {
    khmer: "ពួកយើង",
    roman: "Puak dơng",
    vietnamese: "Chúng mình",
    image: "👫",
  },

  {
    khmer: "លោក",
    roman: "Lôk",
    vietnamese: "Ngài / Ông",
    image: "👨",
  },

  {
    khmer: "អ្នក",
    roman: "Nek",
    vietnamese: "Người",
    image: "🧑",
  },

  {
    khmer: "អ្នកណា",
    roman: "Nek na",
    vietnamese: "Ai",
    image: "❓",
  },

  {
    khmer: "នរណា",
    roman: "Nô na",
    vietnamese: "Người nào",
    image: "❓",
  },

  {
    khmer: "មិត្ត",
    roman: "Mit",
    vietnamese: "Bạn",
    image: "🤝",
  },

  {
    khmer: "មិត្តភក្តិ",
    roman: "Mit-phek",
    vietnamese: "Bạn",
    image: "🤝",
  },

  {
    khmer: "ពួកម៉ាក់",
    roman: "Puak mac",
    vietnamese: "Bạn",
    image: "🤝",
  },

  {
    khmer: "មិត្តសំឡាញ់",
    roman: "Mit som lanh (Som-lanh)",
    vietnamese: "Bạn thân",
    image: "❤️",
  },

  {
    khmer: "សមមិត្ត",
    roman: "Să-mă-mit",
    vietnamese: "Đồng chí",
    image: "🤝",
  },

  {
    khmer: "ឯង",
    roman: "Eng",
    vietnamese: "Mày (nam)",
    image: "👦",
  },

  {
    khmer: "ង៉ែង",
    roman: "Ng'eng",
    vietnamese: "Mày (nữ)",
    image: "👧",
  },

  {
    khmer: "មីនេះ",
    roman: "Mê nis",
    vietnamese: "Nhỏ này",
    image: "👧",
  },

  {
    khmer: "អា",
    roman: "A",
    vietnamese: "Thằng",
    image: "👦",
  },

  {
    khmer: "អា នេះ",
    roman: "A nis",
    vietnamese: "Thằng này",
    image: "👦",
  },

  {
    khmer: "អញ",
    roman: "Anh",
    vietnamese: "Tao",
    image: "🙋",
    note: "Đọc theo giọng Bắc.",
  },

  {
    khmer: "ភ្ញៀវ",
    roman: "Ph'nhiêu",
    vietnamese: "Khách",
    image: "🧳",
  },
];
/* =========================================================
   CHỦ ĐỀ 3: GIAO TIẾP
========================================================= */

const communication = [

  /* =======================================================
     1. CHÀO HỎI
  ======================================================= */

  {
    khmer: "សួស្តី",
    roman: "Suô-sđây",
    vietnamese: "Xin chào",
    image: "👋",
  },

  {
    khmer: "អរុណសួស្តី",
    roman: "A-run suô-sđây",
    vietnamese: "Chào buổi sáng",
    image: "🌅",
  },

  {
    khmer: "សាយ័ណ្ហសួស្តី",
    roman: "Sa-yăn suô-sđây",
    vietnamese: "Chào buổi chiều",
    image: "🌇",
  },

  {
    khmer: "រាត្រីសួស្តី",
    roman: "Ra-trey suô-sđây",
    vietnamese: "Chào buổi tối",
    image: "🌙",
  },

  {
    khmer: "លាហើយ",
    roman: "Lia hâ-y",
    vietnamese: "Tạm biệt",
    image: "👋",
  },

  {
    khmer: "ជួបគ្នាម្តងទៀត",
    roman: "Chuôp knea mdong tiet",
    vietnamese: "Hẹn gặp lại",
    image: "🤝",
  },


  /* =======================================================
     2. CẢM ƠN - XIN LỖI
  ======================================================= */

  {
    khmer: "អរគុណ",
    roman: "O-kun",
    vietnamese: "Cảm ơn",
    image: "🙏",
  },

  {
    khmer: "អរគុណច្រើន",
    roman: "O-kun ch'raơn",
    vietnamese: "Cảm ơn rất nhiều",
    image: "💖",
  },

  {
    khmer: "សូមអភ័យទោស",
    roman: "Som a-phây-tôs",
    vietnamese: "Xin lỗi",
    image: "🙇",
  },

  {
    khmer: "សុំទោស",
    roman: "Som-tôs",
    vietnamese: "Xin lỗi / Cho xin lỗi",
    image: "🙏",
  },

  {
    khmer: "មិនអីទេ",
    roman: "Min ey te",
    vietnamese: "Không sao",
    image: "😊",
  },


  /* =======================================================
     3. HỎI THĂM
  ======================================================= */

  {
    khmer: "សុខសប្បាយទេ?",
    roman: "Sok sa-bay te?",
    vietnamese: "Bạn khỏe không?",
    image: "😊",
  },

  {
    khmer: "ខ្ញុំសុខសប្បាយ",
    roman: "Kh'nhôm sok sa-bay",
    vietnamese: "Tôi khỏe",
    image: "🙂",
  },

  {
    khmer: "អ្នកសុខសប្បាយទេ?",
    roman: "Neak sok sa-bay te?",
    vietnamese: "Bạn có khỏe không?",
    image: "👤",
  },

  {
    khmer: "សុខសប្បាយ",
    roman: "Sok sa-bay",
    vietnamese: "Khỏe / Bình thường",
    image: "👍",
  },


  /* =======================================================
     4. GIỚI THIỆU
  ======================================================= */

  {
    khmer: "តើអ្នកឈ្មោះអ្វី?",
    roman: "Tơ neak ch'muas a-vây?",
    vietnamese: "Bạn tên gì?",
    image: "❓",
  },

  {
    khmer: "ខ្ញុំឈ្មោះ...",
    roman: "Kh'nhôm ch'muas...",
    vietnamese: "Tôi tên là...",
    image: "🙋",
  },

  {
    khmer: "អ្នកមកពីណា?",
    roman: "Neak mok pi na?",
    vietnamese: "Bạn đến từ đâu?",
    image: "📍",
  },

  {
    khmer: "ខ្ញុំមកពីវៀតណាម",
    roman: "Kh'nhôm mok pi Việt Nam",
    vietnamese: "Tôi đến từ Việt Nam",
    image: "🇻🇳",
  },

  {
    khmer: "តើអ្នកជានរណា?",
    roman: "Tơ neak chea no-na?",
    vietnamese: "Bạn là ai?",
    image: "❓",
  },


  /* =======================================================
     5. HỎI ĐÁP CƠ BẢN
  ======================================================= */

  {
    khmer: "នេះជាអ្វី?",
    roman: "Nih chea a-vây?",
    vietnamese: "Đây là gì?",
    image: "👉",
  },

  {
    khmer: "នោះជាអ្វី?",
    roman: "Nuoh chea a-vây?",
    vietnamese: "Kia là gì?",
    image: "👈",
  },

  {
    khmer: "អ្នកយល់ទេ?",
    roman: "Neak yul te?",
    vietnamese: "Bạn hiểu không?",
    image: "🤔",
  },

  {
    khmer: "ខ្ញុំយល់",
    roman: "Kh'nhôm yul",
    vietnamese: "Tôi hiểu",
    image: "💡",
  },

  {
    khmer: "ខ្ញុំមិនយល់ទេ",
    roman: "Kh'nhôm min yul te",
    vietnamese: "Tôi không hiểu",
    image: "😕",
  },

  {
    khmer: "សូមនិយាយយឺតៗ",
    roman: "Som ni-yay yơt-yơt",
    vietnamese: "Xin hãy nói chậm",
    image: "🐢",
  },

  {
    khmer: "សូមនិយាយម្តងទៀត",
    roman: "Som ni-yay mdong tiet",
    vietnamese: "Xin hãy nói lại",
    image: "🔁",
  },

  {
    khmer: "តើអ្នកចេះភាសាខ្មែរទេ?",
    roman: "Tơ neak cheh phia-sa Kh'mae te?",
    vietnamese: "Bạn biết tiếng Khmer không?",
    image: "🇰🇭",
  },

  {
    khmer: "ខ្ញុំនិយាយភាសាខ្មែរបានតិចតួច",
    roman: "Kh'nhôm ni-yay phia-sa Kh'mae ban tech-tuôch",
    vietnamese: "Tôi nói được một ít tiếng Khmer",
    image: "🗣️",
  },


  /* =======================================================
     6. ĐỒNG Ý - TỪ CHỐI
  ======================================================= */

  {
    khmer: "បាទ",
    roman: "Bat",
    vietnamese: "Vâng / Dạ (nam)",
    image: "👍",
  },

  {
    khmer: "ចាស",
    roman: "Chas",
    vietnamese: "Vâng / Dạ (nữ)",
    image: "👍",
  },

  {
    khmer: "បាទ / ចាស",
    roman: "Bat / Chas",
    vietnamese: "Vâng / Dạ",
    image: "👌",
  },

  {
    khmer: "មិនទេ",
    roman: "Min te",
    vietnamese: "Không",
    image: "❌",
  },

  {
    khmer: "បាន",
    roman: "Ban",
    vietnamese: "Được",
    image: "✅",
  },

  {
    khmer: "មិនបានទេ",
    roman: "Min ban te",
    vietnamese: "Không được",
    image: "🚫",
  },


  /* =======================================================
     7. NHỜ VẢ - YÊU CẦU
  ======================================================= */

  {
    khmer: "សូម",
    roman: "Som",
    vietnamese: "Xin / Vui lòng",
    image: "🙏",
  },

  {
    khmer: "សូមជួយខ្ញុំផង",
    roman: "Som chuôi kh'nhôm phong",
    vietnamese: "Xin hãy giúp tôi",
    image: "🆘",
  },

  {
    khmer: "សូមរង់ចាំបន្តិច",
    roman: "Som rong-cham bân-tich",
    vietnamese: "Xin hãy chờ một chút",
    image: "⏳",
  },

  {
    khmer: "សូមអង្គុយ",
    roman: "Som ang-kuy",
    vietnamese: "Xin mời ngồi",
    image: "🪑",
  },

  {
    khmer: "សូមចូល",
    roman: "Som chol",
    vietnamese: "Xin mời vào",
    image: "🚪",
  },


  /* =======================================================
     8. ĐI LẠI - RA VÀO
  ======================================================= */

  {
    khmer: "ចេញ",
    roman: "Chênh",
    vietnamese: "Ra",
    image: "🚪",
  },

  {
    khmer: "ចេញក្រៅ",
    roman: "Chênh krau",
    vietnamese: "Ra ngoài",
    image: "🚶",
  },

  {
    khmer: "សុំចេញក្រៅ",
    roman: "Sôm chênh krau",
    vietnamese: "Xin ra ngoài",
    image: "🚪",
  },

  {
    khmer: "ចូល",
    roman: "Chôl",
    vietnamese: "Vào",
    image: "🚪",
  },

  {
    khmer: "អញ្ជើញចូល",
    roman: "Onh-chơnh chôl",
    vietnamese: "Mời vào",
    image: "👉",
  },

  {
    khmer: "ហាមចូល",
    roman: "Ham chôl",
    vietnamese: "Cấm vào",
    image: "⛔",
  },


  /* =======================================================
     9. HÀNH ĐỘNG THƯỜNG DÙNG
  ======================================================= */

  {
    khmer: "លើក",
    roman: "Lơk",
    vietnamese: "Nâng / Giơ",
    image: "⬆️",
  },

  {
    khmer: "លើកក្បាលឡើង",
    roman: "Lơk k'bal lơng",
    vietnamese: "Nâng đầu lên",
    image: "🙆",
  },

  {
    khmer: "លើកដៃឡើង",
    roman: "Lơk đai lơng",
    vietnamese: "Giơ tay lên",
    image: "🙋",
  },


  /* =======================================================
     10. HẸN - ĐI TRƯỚC / ĐI SAU
  ======================================================= */

  {
    khmer: "ណាត់",
    roman: "Nach",
    vietnamese: "Hẹn",
    image: "📅",
  },

  {
    khmer: "ថ្ងៃក្រោយជួបវិញ",
    roman: "Th'ngay kroi chuôp vinh",
    vietnamese: "Hẹn gặp lại",
    image: "🤝",
  },

  {
    khmer: "មកយឺត",
    roman: "Môk dươt (dư:t)",
    vietnamese: "Đến trễ",
    image: "⏰",
  },

  {
    khmer: "កុំអាលទៅ!",
    roman: "Côm al tâu!",
    vietnamese: "Khoan hãy đi!",
    image: "✋",
  },

  {
    khmer: "ខ្ញុំសុំទៅមុន",
    roman: "Kh'nhôm sôm tâu munh",
    vietnamese: "Tôi xin đi trước",
    image: "🚶",
  },

  {
    khmer: "បងទៅក្រោយណា!",
    roman: "Boong tâu k'roi na!",
    vietnamese: "Anh đi sau nhé!",
    image: "👉",
  },

  {
    khmer: "អូនអង្គុយមុខ",
    roman: "Ône oong-kuy muc",
    vietnamese: "Em ngồi trước",
    image: "🪑",
  },

  {
    khmer: "បងអង្គុយក្រោយ",
    roman: "Boong oong-kuy k'roi",
    vietnamese: "Anh ngồi sau",
    image: "🪑",
  },


  /* =======================================================
     11. CHÚC NGỦ NGON
  ======================================================= */

  {
    khmer: "ដេកស្រួល",
    roman: "Đêk srual",
    vietnamese: "Ngủ ngon",
    image: "😴",
  },

  {
    khmer: "ជូនពរ សុខបក្សល្អ",
    roman: "Chun pô Sôk-bân-lô",
    vietnamese: "Chúc ngủ ngon",
    image: "🌙",
  },


  /* =======================================================
     12. CHÚC - CÂU GIAO TIẾP
  ======================================================= */

  {
    khmer: "សូមឲ្យមានសំណាងល្អ",
    roman: "Som oy mean sam-nang la-or",
    vietnamese: "Chúc may mắn",
    image: "🍀",
  },

  {
    khmer: "សូមឲ្យមានសុខភាពល្អ",
    roman: "Som oy mean sok-kha-pheap la-or",
    vietnamese: "Chúc bạn mạnh khỏe",
    image: "❤️",
  },

  {
    khmer: "រីករាយណាស់ដែលបានជួបអ្នក",
    roman: "Rik-reay nas del ban chuôp neak",
    vietnamese: "Rất vui được gặp bạn",
    image: "😊",
  },

  {
    khmer: "សូមស្វាគមន៍",
    roman: "Som sva-kum",
    vietnamese: "Chào mừng",
    image: "🎉",
  },
];
/* =========================================================
   CHỦ ĐỀ 3: GIA ĐÌNH & QUAN HỆ
   TT: 19 → 77
========================================================= */

const family = [
  {
    khmer: "ផ្ទះ",
    roman: "Ph'tes",
    vietnamese: "Nhà",
    image: "🏠",
  },

  {
    khmer: "គ្រួសារ",
    roman: "Krua-sa",
    vietnamese: "Gia đình",
    image: "👨‍👩‍👧‍👦",
  },

  {
    khmer: "ឪពុកម្ដាយ / ឪម៉ែ / ប៉ាម៉ាក់",
    roman: "Âu púc-m'đai / Âu-me / Pa-măk",
    vietnamese: "Cha / mẹ",
    image: "👨‍👩‍👧",
  },

  {
    khmer: "តា (លោកតា)",
    roman: "Ta (lôk ta)",
    vietnamese: "Ông",
    image: "👴",
  },

  {
    khmer: "យាយ (លោកយាយ)",
    roman: "Diay (lôk diay)",
    vietnamese: "Bà",
    image: "👵",
  },

  {
    khmer: "ខាងឪពុក",
    roman: "Khang âu puc",
    vietnamese: "Bên nội",
    image: "👨",
  },

  {
    khmer: "ខាងម្ដាយ",
    roman: "Khang m'đai",
    vietnamese: "Bên ngoại",
    image: "👩",
  },

  {
    khmer: "តាខាងឪពុក",
    roman: "Ta khang âu puc",
    vietnamese: "Ông nội",
    image: "👴",
  },

  {
    khmer: "តាខាងម្ដាយ",
    roman: "Ta khang m'đai",
    vietnamese: "Ông ngoại",
    image: "👴",
  },

  {
    khmer: "យាយខាងឪពុក",
    roman: "Diay khang âu puc",
    vietnamese: "Bà nội",
    image: "👵",
  },

  {
    khmer: "យាយខាងម្ដាយ",
    roman: "Diay khang m'đai",
    vietnamese: "Bà ngoại",
    image: "👵",
  },

  {
    khmer: "ពូ (លោកពូ)",
    roman: "Pu (lôk pu)",
    vietnamese: "Chú",
    image: "👨",
  },

  {
    khmer: "អ៊ុំ (លោកអ៊ុំ)",
    roman: "Ôm (lôk ôm)",
    vietnamese: "Bác",
    image: "👨",
  },

  {
    khmer: "ម្តាយមីង / មា",
    roman: "Mia",
    vietnamese: "Cậu",
    image: "👨",
  },

  {
    khmer: "មីង",
    roman: "Mi:ng",
    vietnamese: "Cô / Dì",
    image: "👩",
    note: 'Mi:ng: đọc "i:" dài.',
  },

  {
    khmer: "បង",
    roman: "Boong",
    vietnamese: "Anh",
    image: "👦",
  },

  {
    khmer: "បងស្រី",
    roman: "Boong Srây",
    vietnamese: "Chị",
    image: "👧",
  },

  {
    khmer: "អូន / ប្អូន",
    roman: "Ône / P'ône",
    vietnamese: "Em",
    image: "🧒",
  },

  {
    khmer: "ប្រពន្ធ",
    roman: "Pro-pôn",
    vietnamese: "Vợ",
    image: "👩",
  },

  {
    khmer: "ប្តី",
    roman: "Pa-đây",
    vietnamese: "Chồng",
    image: "👨",
  },

  {
    khmer: "កូន",
    roman: "Cône",
    vietnamese: "Con",
    image: "👶",
  },

  {
    khmer: "កូនច្បង",
    roman: "Cône ch'boong",
    vietnamese: "Con cả",
    image: "👦",
  },

  {
    khmer: "កូនពៅ",
    roman: "Cône pâu",
    vietnamese: "Con út",
    image: "👶",
  },

  {
    khmer: "កូនប្រុស",
    roman: "Cône prôs",
    vietnamese: "Con trai",
    image: "👦",
  },

  {
    khmer: "កូនស្រី",
    roman: "Cône srây",
    vietnamese: "Con gái",
    image: "👧",
  },

  {
    khmer: "កូនប្រសាប្រុស",
    roman: "Cône pro-sa prôs",
    vietnamese: "Con rể",
    image: "👨",
  },

  {
    khmer: "កូនប្រសាស្រី",
    roman: "Cône pro-sa srây",
    vietnamese: "Con dâu",
    image: "👩",
  },

  {
    khmer: "បងថ្លៃ",
    roman: "Boong th'lay",
    vietnamese: "Anh rể",
    image: "👨",
  },

  {
    khmer: "បងស្រីថ្លៃ",
    roman: "Boong srây th'lay",
    vietnamese: "Chị dâu",
    image: "👩",
  },

  {
    khmer: "ចៅ",
    roman: "Chau",
    vietnamese: "Cháu",
    image: "👶",
    note: "Ông bà xưng với cháu.",
  },

  {
    khmer: "ចៅប្រុស",
    roman: "Chau prôs",
    vietnamese: "Cháu trai",
    image: "👦",
  },

  {
    khmer: "ចៅស្រី",
    roman: "Chau srây",
    vietnamese: "Cháu gái",
    image: "👧",
  },

  {
    khmer: "ក្មួយ",
    roman: "Kh' muôi",
    vietnamese: "Cháu",
    image: "🧒",
    note: "Chú, bác, cô, dì xưng với cháu.",
  },

  {
    khmer: "ក្មួយប្រុស",
    roman: "Kh' muôi prôs",
    vietnamese: "Cháu trai",
    image: "👦",
  },

  {
    khmer: "ក្មួយស្រី",
    roman: "Kh' muôi srây",
    vietnamese: "Cháu gái",
    image: "👧",
  },

  {
    khmer: "ធម៌ / ចិញ្ចឹម",
    roman: "Thoa / chênh châm",
    vietnamese: "Nuôi",
    image: "🤲",
  },

  {
    khmer: "ឪពុកធម៌ / ឪធម៌ / ប៉ាធម៌",
    roman: "Âu púc thoa / Âu thoa / Pa thoa",
    vietnamese: "Cha nuôi",
    image: "👨",
  },

  {
    khmer: "ម្ដាយធម៌ / ម៉ែធម៌ / ម៉ាក់ធម៌",
    roman: "M'đai thoa / Me thoa / Măk thoa",
    vietnamese: "Mẹ nuôi",
    image: "👩",
  },

  {
    khmer: "កូនធម៌",
    roman: "Cône thoa",
    vietnamese: "Con nuôi",
    image: "🧒",
  },

  {
    khmer: "បងធម៌",
    roman: "Boong thoa",
    vietnamese: "Anh nuôi",
    image: "👦",
  },

  {
    khmer: "បងស្រីធម៌",
    roman: "Boong srây thoa",
    vietnamese: "Chị nuôi",
    image: "👧",
  },

  {
    khmer: "បង្កើត",
    roman: "Boong-cơt",
    vietnamese: "Ruột",
    image: "👨‍👩‍👧",
  },

  {
    khmer: "កូនបង្កើត",
    roman: "Cône boong-cơt",
    vietnamese: "Con ruột",
    image: "👶",
  },

  {
    khmer: "បងបង្កើត",
    roman: "Boong boong-cơt",
    vietnamese: "Anh ruột",
    image: "👦",
  },

  {
    khmer: "បងស្រីបង្កើត",
    roman: "Boong srây boong-cơt",
    vietnamese: "Chị ruột",
    image: "👧",
  },

  {
    khmer: "បងប្អូនបង្កើត",
    roman: "Boong-p'ône boong-cơt",
    vietnamese: "Anh em ruột",
    image: "👫",
  },

  {
    khmer: "បងប្អូន / សាច់ញាតិ",
    roman: "Boong-p'ône / Sach nhiêt",
    vietnamese: "Bà con / Họ hàng",
    image: "👨‍👩‍👧‍👦",
  },

  {
    khmer: "ទៅសួរសុខទុក្ខបងប្អូន",
    roman: "Tâu sua sôk tuc boong-p'ône",
    vietnamese: "Đi thăm bà con",
    image: "🚶",
  },

  {
    khmer: "ភេទ",
    roman: "Phết",
    vietnamese: "Giới tính",
    image: "⚧️",
  },

  {
    khmer: "ប្រុស",
    roman: "Prôs",
    vietnamese: "Trai",
    image: "👦",
  },

  {
    khmer: "ស្រី",
    roman: "Srây",
    vietnamese: "Gái",
    image: "👧",
  },

  {
    khmer: "ក្មេង",
    roman: "Kh'mêng",
    vietnamese: "Trẻ / Trẻ con",
    image: "🧒",
  },

  {
    khmer: "កុមារ",
    roman: "Côk ma",
    vietnamese: "Trẻ em",
    image: "👧",
  },

  {
    khmer: "ទារក",
    roman: "Tia rúk",
    vietnamese: "Trẻ sơ sinh",
    image: "👶",
  },

  {
    khmer: "គភ៌",
    roman: "Koa",
    vietnamese: "Thai nhi",
    image: "🤰",
  },

  {
    khmer: "មនុស្ស",
    roman: "Mô-nus",
    vietnamese: "Con người",
    image: "🧑",
  },

  {
    khmer: "លីវ",
    roman: "Liêu",
    vietnamese: "Độc thân",
    image: "🧑",
  },

  {
    khmer: "នៅលីវ",
    roman: "Nâu liêu",
    vietnamese: "Còn độc thân",
    image: "🧍",
  },

  {
    khmer: "មានគ្រួសារហើយ",
    roman: "Mian krua-sa hơi",
    vietnamese: "Có gia đình rồi",
    image: "💑",
  },
];
/* =========================================================
   CHỦ ĐỀ 5: NGHỀ NGHIỆP & CHỨC VỤ
========================================================= */

const jobs = [
  /* -------------------------------------------------------
     CHỨC VỤ
  ------------------------------------------------------- */

  {
    khmer: "មេ / ប្រធាន / នាយក",
    roman: "Mê / Prothian / Niaduk",
    vietnamese: "Sếp",
    image: "👔",
  },

  {
    khmer: "អនុ / រង",
    roman: "A nuk / Rôông",
    vietnamese: "Phó",
    image: "👔",
  },

  {
    khmer: "នាយក",
    roman: "Nia duak",
    vietnamese: "Giám đốc",
    image: "🏢",
  },

  {
    khmer: "នាយករង",
    roman: "Nia duak rôông",
    vietnamese: "Phó giám đốc",
    image: "🏢",
  },

  {
    khmer: "ប្រធានការិយាល័យ",
    roman: "Prothian ca-ri-da-lay",
    vietnamese: "Trưởng phòng",
    image: "💼",
  },

  {
    khmer: "អនុប្រធានការិយាល័យ",
    roman: "A nuk Prothian ca-ri-da-lay",
    vietnamese: "Phó trưởng phòng",
    image: "💼",
  },

  {
    khmer: "ប្រធានផ្នែក",
    roman: "Prothian ph'nêk",
    vietnamese: "Trưởng khoa",
    image: "📋",
  },

  {
    khmer: "អនុប្រធានផ្នែក",
    roman: "A nuk Prothian ph'nêk",
    vietnamese: "Phó trưởng khoa",
    image: "📋",
  },

  /* -------------------------------------------------------
     TRƯỜNG HỌC
  ------------------------------------------------------- */

  {
    khmer: "គ្រូបង្រៀន",
    roman: "Kru boong-rian",
    vietnamese: "Giáo viên",
    image: "👨‍🏫",
  },

  {
    khmer: "លោកគ្រូ",
    roman: "Lôk Kru",
    vietnamese: "Thầy giáo",
    image: "👨‍🏫",
  },

  {
    khmer: "អ្នកគ្រូ",
    roman: "Nek Kru",
    vietnamese: "Cô giáo",
    image: "👩‍🏫",
  },

  {
    khmer: "មេថ្នាក់ / ប្រធានថ្នាក់",
    roman: "Mê th'nắk / Prothian thnăk",
    vietnamese: "Lớp trưởng",
    image: "🎓",
  },

  {
    khmer: "មេថ្នាក់រង / អនុប្រធានថ្នាក់",
    roman: "Mê th'nắk rôông / A nuk Prothian thnăk",
    vietnamese: "Lớp phó",
    image: "🎓",
  },

  {
    khmer: "បេឡាធិការថ្នាក់",
    roman: "Bê-la thi-ca thnăk",
    vietnamese: "Thủ quỹ",
    image: "💰",
  },

  {
    khmer: "គណនេយ្យ",
    roman: "Kek-nă-nây",
    vietnamese: "Kế toán",
    image: "🧮",
  },

  {
    khmer: "គណនេយ្យករ",
    roman: "Kek-nă-nây dek-co",
    vietnamese: "Kế toán viên",
    image: "🧮",
  },

  {
    khmer: "សិក្ខាកាម",
    roman: "Sấc-kha-cam",
    vietnamese: "Học viên",
    image: "📚",
  },

  {
    khmer: "បេក្ខជន",
    roman: "Pê-khă-chônh",
    vietnamese: "Thí sinh / Ứng viên",
    image: "📝",
  },

  /* -------------------------------------------------------
     CÔNG AN ĐỊA PHƯƠNG
  ------------------------------------------------------- */

  {
    khmer: "ប្រធាននគរបាលឃុំ",
    roman: "Pro-thian nô-cô-bal khum",
    vietnamese: "Trưởng Công an xã",
    image: "👮",
  },

  {
    khmer: "អនុប្រធាននគរបាលឃុំ",
    roman: "A-nuk pro-thian nô-cô-bal khum",
    vietnamese: "Phó Trưởng Công an xã",
    image: "👮",
  },

  {
    khmer: "ប្រធាននគរបាលសង្កាត់",
    roman: "Pro-thian nô-cô-bal sang-kat",
    vietnamese: "Trưởng Công an phường",
    image: "👮",
  },

  {
    khmer: "អនុប្រធាននគរបាលសង្កាត់",
    roman: "A-nuk pro-thian nô-cô-bal sang-kat",
    vietnamese: "Phó Trưởng Công an phường",
    image: "👮",
  },

  {
    khmer: "បុគ្គលិកនគរបាល / មន្ត្រីនគរបាល",
    roman: "Bôk-kek-lưc nô-cô-bal / Môn-trây nô-cô-bal",
    vietnamese: "Công an viên",
    image: "👮",
  },
];
/* =========================================================
   CHỦ ĐỀ 6: CƠ THỂ NGƯỜI
========================================================= */

const body = [

  /* =======================================================
     1. ĐẦU & MẶT
  ======================================================= */

  {
    khmer: "ក្បាល",
    roman: "K'bal",
    vietnamese: "Đầu",
    image: "🧠",
  },
  {
    khmer: "សក់",
    roman: "Sok",
    vietnamese: "Tóc",
    image: "💇",
  },
  {
    khmer: "ខួរ",
    roman: "Khua",
    vietnamese: "Não (Óc)",
    image: "🧠",
  },
  {
    khmer: "ថ្ងាស",
    roman: "Th'ngas",
    vietnamese: "Trán",
    image: "🙂",
  },
  {
    khmer: "ភ្នែក",
    roman: "Ph'nêk",
    vietnamese: "Mắt",
    image: "👁️",
  },
  {
    khmer: "ចិញ្ចើម",
    roman: "Chênh-chơm",
    vietnamese: "Lông mày",
    image: "👁️",
  },
  {
    khmer: "រោមភ្នែក",
    roman: "Rôm ph'nêk",
    vietnamese: "Lông mi",
    image: "👁️",
  },
  {
    khmer: "ត្របកភ្នែក",
    roman: "T'ro-bok ph'nêk",
    vietnamese: "Mí mắt",
    image: "👁️",
  },
  {
    khmer: "ច្រមុះ",
    roman: "Ch'ro-môs",
    vietnamese: "Mũi",
    image: "👃",
  },
  {
    khmer: "រន្ធច្រមុះ",
    roman: "Rônh ch'ro-môs",
    vietnamese: "Lỗ mũi",
    image: "👃",
  },
  {
    khmer: "មាត់",
    roman: "Mo:th",
    vietnamese: "Miệng",
    image: "👄",
  },
  {
    khmer: "បបូរមាត់",
    roman: "Bo-bô mo:th",
    vietnamese: "Môi",
    image: "👄",
  },
  {
    khmer: "អណ្តាត",
    roman: "Onh-đath",
    vietnamese: "Lưỡi",
    image: "👅",
  },
  {
    khmer: "ទឹកមាត់",
    roman: "Tưc mo:th",
    vietnamese: "Nước bọt",
    image: "💧",
  },
  {
    khmer: "ក្រពេញទឹកមាត់",
    roman: "K'ro-pênh tưc mo:th",
    vietnamese: "Tuyến nước bọt",
    image: "💧",
  },
  {
    khmer: "ក្រពេញខាន់ស្លាក់",
    roman: "K'ro-pênh khanh s'lăk",
    vietnamese: "Tuyến giáp",
    image: "🫁",
  },
  {
    khmer: "ធ្មេញ",
    roman: "Th'mênh",
    vietnamese: "Răng",
    image: "🦷",
  },
  {
    khmer: "អញ្ចាញធ្មេញ",
    roman: "Onh-chanh th'mênh",
    vietnamese: "Nướu răng",
    image: "🦷",
  },
  {
    khmer: "ថ្គាម",
    roman: "Th'kiam",
    vietnamese: "Hàm",
    image: "🙂",
  },
  {
    khmer: "មុខ",
    roman: "Muc",
    vietnamese: "Mặt",
    image: "🙂",
  },
  {
    khmer: "ពុកមាត់",
    roman: "Puc mo:th",
    vietnamese: "Ria",
    image: "🥸",
  },
  {
    khmer: "ចង្ការ",
    roman: "Choong-ca",
    vietnamese: "Cằm",
    image: "🙂",
    note: '“Choong” đọc ngắn.',
  },
  {
    khmer: "ពុកចង្ការ",
    roman: "Puc Choong-ca",
    vietnamese: "Râu cằm",
    image: "🥸",
  },
  {
    khmer: "ថ្ពាល់",
    roman: "Th'pol",
    vietnamese: "Má",
    image: "🙂",
  },
  {
    khmer: "ត្រចៀក",
    roman: "T'ro-chiak",
    vietnamese: "Tai",
    image: "👂",
  },
  {
    khmer: "រន្ធត្រចៀក",
    roman: "Rônh T'ro-chiak",
    vietnamese: "Lỗ tai",
    image: "👂",
  },
  {
    khmer: "ស្លឹកត្រចៀក",
    roman: "Slâc T'ro-chiak",
    vietnamese: "Vành tai",
    image: "👂",
  },
  {
    khmer: "ក្រដាសត្រចៀក",
    roman: "K'ro-đas T'ro-chiak",
    vietnamese: "Màng nhĩ",
    image: "👂",
  },

  /* =======================================================
     2. CỔ & CHI TRÊN
  ======================================================= */

  {
    khmer: "ក",
    roman: "Co",
    vietnamese: "Cổ",
    image: "🧍",
  },
  {
    khmer: "បំពង់ក",
    roman: "Bom-pung co",
    vietnamese: "Cổ họng (họng)",
    image: "🫁",
  },
  {
    khmer: "បំពង់ខ្យល់",
    roman: "Bom-pung K'dol",
    vietnamese: "Khí quản",
    image: "🫁",
  },
  {
    khmer: "បំពុងអាហារ",
    roman: "Bom-pung a-ha",
    vietnamese: "Thực quản",
    image: "🫁",
  },
  {
    khmer: "ស្មា",
    roman: "S'ma",
    vietnamese: "Vai",
    image: "💪",
  },
  {
    khmer: "ដៃ",
    roman: "Đai",
    vietnamese: "Tay",
    image: "✋",
  },
  {
    khmer: "ក្លៀក",
    roman: "Kh'liak",
    vietnamese: "Nách",
    image: "💪",
  },
  {
    khmer: "កំភូនដៃ",
    roman: "Com-phuan đai",
    vietnamese: "Bắp tay",
    image: "💪",
  },
  {
    khmer: "សាច់ដុំ",
    roman: "Sach đôm",
    vietnamese: "Cơ bắp",
    image: "💪",
  },
  {
    khmer: "កែងដៃ",
    roman: "Keng đai",
    vietnamese: "Khuỷu tay",
    image: "💪",
  },
  {
    khmer: "កដៃ",
    roman: "Co đai",
    vietnamese: "Cổ tay",
    image: "⌚",
  },
  {
    khmer: "បាតដៃ",
    roman: "Bath đai",
    vietnamese: "Lòng bàn tay",
    image: "✋",
  },
  {
    khmer: "ម្រាមដៃ",
    roman: "M'riam đai",
    vietnamese: "Ngón tay",
    image: "☝️",
  },
  {
    khmer: "ក្រចកដៃ",
    roman: "K'ro-chok đai",
    vietnamese: "Móng tay",
    image: "💅",
  },
  {
    khmer: "រោមដៃ",
    roman: "Rôm đai",
    vietnamese: "Lông tay",
    image: "💪",
  },

  /* =======================================================
     3. THÂN MÌNH & NỘI TẠNG
  ======================================================= */

  {
    khmer: "ទ្រួង",
    roman: "T'ru:ng",
    vietnamese: "Ngực (lồng ngực)",
    image: "🫁",
    note: 'Đọc “u” dài.',
  },
  {
    khmer: "ដោះ / សុដន់",
    roman: "Đos / sôk đonh",
    vietnamese: "Vú (ngực)",
    image: "🫶",
  },
  {
    khmer: "ចុងដោះ / ក្បាលសុដន់",
    roman: "Chôông đos / K'bal sôk đonh",
    vietnamese: "Nhũ hoa (núm vú)",
    image: "🫶",
  },
  {
    khmer: "បេះដូង",
    roman: "Bês đôông",
    vietnamese: "Tim",
    image: "❤️",
    note: 'Đọc “ô” dài.',
  },
  {
    khmer: "សន្ទះបេះដូង",
    roman: "Son-tes Bês đôông",
    vietnamese: "Van tim",
    image: "❤️",
  },
  {
    khmer: "សួត",
    roman: "Suat (suôth)",
    vietnamese: "Phổi",
    image: "🫁",
  },
  {
    khmer: "ខ្នង",
    roman: "Kh'noong",
    vietnamese: "Lưng / Thắt lưng",
    image: "🧍",
  },
  {
    khmer: "ឆ្អឺង",
    roman: "Chh'âng",
    vietnamese: "Xương",
    image: "🦴",
  },
  {
    khmer: "ឆ្អឺងខ្នង",
    roman: "Chh'âng Kh'noong",
    vietnamese: "Cột sống",
    image: "🦴",
  },
  {
    khmer: "ពោះ",
    roman: "Puas",
    vietnamese: "Bụng",
    image: "🧍",
  },
  {
    khmer: "ថ្លៀម",
    roman: "Th'lơm",
    vietnamese: "Gan",
    image: "🫀",
  },
  {
    khmer: "ប្រមាត់",
    roman: "Pro-măth / P'măth",
    vietnamese: "Mật",
    image: "🫀",
  },
  {
    khmer: "ថង់ទឹកប្រមាត់",
    roman: "Thoong tưc Pro-măth",
    vietnamese: "Túi mật",
    image: "🫀",
  },
  {
    khmer: "ក្រពះ",
    roman: "K'ro-pes / K'pes",
    vietnamese: "Dạ dày / Bao tử",
    image: "🫃",
  },
  {
    khmer: "បំពង់ពោះវៀនដំបូង",
    roman: "Bom-pung puas vian đom-bôn",
    vietnamese: "Tá tràng",
    image: "🫃",
  },
  {
    khmer: "លំពែង",
    roman: "Lôm-peng",
    vietnamese: "Tụy (Lá mía)",
    image: "🫀",
  },
  {
    khmer: "អណ្តើក",
    roman: "Onh-đơk",
    vietnamese: "Lá lách",
    image: "🫀",
    note: 'Ngoài ra có nghĩa là “con rùa”.',
  },
  {
    khmer: "ពោះវៀន",
    roman: "Puas-vian",
    vietnamese: "Ruột",
    image: "🫃",
  },
  {
    khmer: "ពោះវៀនតូច",
    roman: "Puas-vian tôch",
    vietnamese: "Ruột non",
    image: "🫃",
  },
  {
    khmer: "ពោះវៀនធំ",
    roman: "Puas-vian thôm",
    vietnamese: "Ruột già",
    image: "🫃",
  },
  {
    khmer: "ខ្នែងពោះវៀន",
    roman: "Kh'neng Puas-vian",
    vietnamese: "Ruột thừa",
    image: "🫃",
  },
  {
    khmer: "ពោះវៀនត្រង់",
    roman: "Puas-vian t'roong",
    vietnamese: "Trực tràng",
    image: "🫃",
  },
  {
    khmer: "ផ្ចិត",
    roman: "Ph'cheth",
    vietnamese: "Rốn",
    image: "🧍",
    note: "Cũng có cách đọc Ph'seth.",
  },
  {
    khmer: "គូថ",
    roman: "Ku:th",
    vietnamese: "Mông",
    image: "🧍",
    note: 'Đọc “u” dài.',
  },

  /* =======================================================
     4. CHI DƯỚI
  ======================================================= */

  {
    khmer: "ជើង",
    roman: "Chơng",
    vietnamese: "Chân",
    image: "🦵",
  },
  {
    khmer: "ភ្លៅ",
    roman: "Ph'lâu",
    vietnamese: "Đùi",
    image: "🦵",
  },
  {
    khmer: "ក្បាលជង្គង់ / ជង្គង់",
    roman: "K'bal chôông-cung",
    vietnamese: "Đầu gối",
    image: "🦵",
  },
  {
    khmer: "កំភូនជើង",
    roman: "Com-phuan chơng",
    vietnamese: "Bắp chân",
    image: "🦵",
  },
  {
    khmer: "ស្មងជើង",
    roman: "S'moong chơng",
    vietnamese: "Ống chân",
    image: "🦵",
  },
  {
    khmer: "ភ្នែកគោរ",
    roman: "Ph'neek cô",
    vietnamese: "Mắt cá chân",
    image: "🦶",
  },
  {
    khmer: "កជើង",
    roman: "Co chơng",
    vietnamese: "Cổ chân",
    image: "🦶",
  },
  {
    khmer: "ប្រអប់ជើង",
    roman: "P'ro-op chơng",
    vietnamese: "Mu bàn chân",
    image: "🦶",
  },
  {
    khmer: "បាតជើង",
    roman: "Bath chơng",
    vietnamese: "Lòng bàn chân",
    image: "🦶",
  },
  {
    khmer: "ម្រាមជើង",
    roman: "M'riam chơng",
    vietnamese: "Ngón chân",
    image: "🦶",
  },
  {
    khmer: "មេជើង",
    roman: "Mê chơng",
    vietnamese: "Ngón chân cái",
    image: "🦶",
  },
  {
    khmer: "ម្រាមចង្អុល",
    roman: "M'riam chôông-ôl",
    vietnamese: "Ngón chân trỏ",
    image: "🦶",
  },
  {
    khmer: "ម្រាមកណ្ដាល",
    roman: "M'riam kon-đal",
    vietnamese: "Ngón chân giữa",
    image: "🦶",
  },
  {
    khmer: "ម្រាមនាងជើង",
    roman: "M'riam niêng chơng",
    vietnamese: "Ngón chân áp út",
    image: "🦶",
  },
  {
    khmer: "ម្រាមកូនជើង",
    roman: "Cône-chơng",
    vietnamese: "Ngón chân út",
    image: "🦶",
  },
  {
    khmer: "កែងជើង",
    roman: "Keng chơng",
    vietnamese: "Gót chân",
    image: "🦶",
  },
  {
    khmer: "ក្រចកជើង",
    roman: "K'ro-chok chơng",
    vietnamese: "Móng chân",
    image: "🦶",
  },
  {
    khmer: "រោមជើង",
    roman: "Rôm chơng",
    vietnamese: "Lông chân",
    image: "🦵",
  },

  /* =======================================================
     5. TIẾT NIỆU, SINH DỤC & BÀI TIẾT
  ======================================================= */

  {
    khmer: "លិង",
    roman: "Lưng",
    vietnamese: "Dương vật",
    image: "🧍",
  },
  {
    khmer: "ក្បាលលិង",
    roman: "K'bal lưng",
    vietnamese: "Quy đầu",
    image: "🧍",
  },
  {
    khmer: "តម្រងនោម",
    roman: "Đom-roong nôm",
    vietnamese: "Niệu quản",
    image: "🫘",
  },
  {
    khmer: "ប្លោកនោម",
    roman: "Ph'lok nôm",
    vietnamese: "Bàng quang / Bọng đái",
    image: "🫧",
  },
  {
    khmer: "ក្រលៀន",
    roman: "K'ro-lian",
    vietnamese: "Thận (Quả thận)",
    image: "🫘",
  },
  {
    khmer: "ទ្វារមាស",
    roman: "Th'via mias",
    vietnamese: "Âm hộ",
    image: "🧍",
  },
  {
    khmer: "រន្ធទ្វារមាស",
    roman: "Rônh Th'via mias",
    vietnamese: "Âm đạo",
    image: "🧍",
  },
  {
    khmer: "ស្បូន",
    roman: "S'bôn",
    vietnamese: "Tử cung",
    image: "🧍",
  },
  {
    khmer: "ដៃស្បូន",
    roman: "Đai s'bôn",
    vietnamese: "Tai vòi",
    image: "🧍",
  },
  {
    khmer: "កន្សោមកូន",
    roman: "Conh-som cône / Ôve",
    vietnamese: "Buồng trứng",
    image: "🧍",
  },
  {
    khmer: "ទ្វារបាត",
    roman: "Th'vía bath",
    vietnamese: "Hậu môn",
    image: "🧍",
  },
];
/* =========================================================
   CHỦ ĐỀ 7: ĐỊA CHỈ – GIẤY TỜ – LIÊN HỆ & HÀNH CHÍNH
========================================================= */

const addressAdmin = [
  /* =======================================================
     I. ĐỊA CHỈ & ĐƠN VỊ HÀNH CHÍNH
  ======================================================= */

  {
    khmer: "អាសយដ្ឋាន",
    roman: "A-sây-dek-than",
    vietnamese: "Địa chỉ",
    image: "📍",
    note: "Có thể đọc: A-sây-da-than.",
  },

  {
    khmer: "លេខផ្ទះ",
    roman: "Lêk ph'tes",
    vietnamese: "Số nhà",
    image: "🏠",
  },

  {
    khmer: "ផ្លូវ",
    roman: "Ph'lâu",
    vietnamese: "Đường",
    image: "🛣️",
  },

  {
    khmer: "ផ្លូវជាតិ",
    roman: "Ph'lâu chiêt",
    vietnamese: "Quốc lộ",
    image: "🛣️",
  },

  {
    khmer: "ភូមិ",
    roman: "Phu:m",
    vietnamese: "Ấp / Xóm / Làng",
    image: "🏘️",
    note: 'Đọc "u" kéo dài.',
  },

  {
    khmer: "ឃុំ",
    roman: "Khum",
    vietnamese: "Xã",
    image: "🏘️",
  },

  {
    khmer: "ស្រុក",
    roman: "Srôk",
    vietnamese: "Huyện",
    image: "🏢",
  },

  {
    khmer: "ខេត្ត",
    roman: "Khet",
    vietnamese: "Tỉnh",
    image: "🏛️",
  },

  {
    khmer: "ទីក្រុង",
    roman: "Ti-crô:ng",
    vietnamese: "Thành phố",
    image: "🌆",
  },

  {
    khmer: "រាជធានី / រាជធានី",
    roman: "Ro:th-thia-ni / Riach-thia-ni",
    vietnamese: "Thủ đô / Kinh đô",
    image: "🏙️",
  },

  {
    khmer: "ទីរួមស្រុក",
    roman: "Ti-ruôm Srôk",
    vietnamese: "Thị trấn",
    image: "🏘️",
  },

  {
    khmer: "ទីរួមខេត្ត",
    roman: "Ti-ruôm Khet",
    vietnamese: "Thị xã",
    image: "🏙️",
  },

  {
    khmer: "ខណ្ឌ",
    roman: "Khanh",
    vietnamese: "Quận",
    image: "🏢",
  },

  {
    khmer: "សង្កាត់",
    roman: "So:ng-căth",
    vietnamese: "Phường",
    image: "🏘️",
  },

  {
    khmer: "អនុសង្កាត់",
    roman: "Ă-nuk so:ng-căth",
    vietnamese: "Khóm",
    image: "🏠",
  },

  /* =======================================================
     II. TÊN ĐỊA DANH
  ======================================================= */

  {
    khmer: "ព្រៃនគរ",
    roman: "Prây-nô-cô",
    vietnamese: "TP. Hồ Chí Minh",
    image: "🌆",
  },

  {
    khmer: "ព្រែកឫស្សី",
    roman: "Prêk-rưs-sây",
    vietnamese: "Cần Thơ",
    image: "🌆",
  },

  {
    khmer: "បារាជ",
    roman: "Pa-ra:ch / Ba-ra:ch",
    vietnamese: "Long Xuyên",
    image: "🏙️",
  },

  {
    khmer: "មាត់ជ្រូក",
    roman: "Mo:th-chru:k",
    vietnamese: "Châu Đốc",
    image: "🏙️",
  },

  {
    khmer: "ស្វាយទង",
    roman: "Svay-tô:n",
    vietnamese: "Tri Tôn",
    image: "🌄",
  },

  {
    khmer: "កាបៅ",
    roman: "Ka-bau",
    vietnamese: "Tịnh Biên",
    image: "🌄",
  },

  {
    khmer: "ភ្នំស្វាម",
    roman: "Ph'nôm s'vam",
    vietnamese: "Núi Sam",
    image: "⛰️",
  },

  /* =======================================================
     III. GIẤY TỜ & THỦ TỤC HÀNH CHÍNH
  ======================================================= */

  {
    khmer: "ស្រុកកំណើត",
    roman: "Srôk com-nơt",
    vietnamese: "Quê quán",
    image: "🏡",
  },

  {
    khmer: "កន្លែងកំណើត",
    roman: "Con-leng com-nơ:t",
    vietnamese: "Nơi sinh",
    image: "📍",
    note: 'Đọc "ơ" kéo dài.',
  },

  {
    khmer: "ស្នាក់នៅបណ្ដោះអាសន្ន",
    roman: "Snăk nâu bon-đos a-sonh",
    vietnamese: "Thường trú",
    image: "🏠",
  },

  {
    khmer: "ថ្ងៃកំណើត",
    roman: "Th'ngay com-nơ:t",
    vietnamese: "Ngày sinh / Sinh nhật",
    image: "🎂",
  },

  {
    khmer: "ជនជាតិ",
    roman: "Chôn-chiat",
    vietnamese: "Dân tộc",
    image: "👥",
    note: "Có thể đọc: chiêt.",
  },

  {
    khmer: "សញ្ជាតិ",
    roman: "Sanh-chiat",
    vietnamese: "Quốc tịch",
    image: "🌍",
    note: "Có thể đọc: chiêt.",
  },

  {
    khmer: "អត្តសញ្ញាណប័ណ្ណ",
    roman: "A-tă-sanh-nhian-nă-banh",
    vietnamese: "CCCD / CMND",
    image: "🪪",
  },

  {
    khmer: "សំបុត្រកំណើត",
    roman: "Som-bôth com-nơt",
    vietnamese: "Giấy khai sinh",
    image: "📄",
  },

  {
    khmer: "សំបុត្របញ្ជាក់កំណើត",
    roman: "Som-bôth banh-chek com-nơt",
    vietnamese: "Giấy chứng sinh",
    image: "📄",
  },

  {
    khmer: "សៀវភៅគ្រួសារ",
    roman: "Siêu phâu krua-sa",
    vietnamese: "Hộ khẩu",
    image: "📖",
  },

  {
    khmer: "លិខិតឆ្លងដែន",
    roman: "Pas-po",
    vietnamese: "Hộ chiếu",
    image: "🛂",
    note: "Passport.",
  },

  {
    khmer: "ប័ណ្ណបើកបរ",
    roman: "Banh bơk bo",
    vietnamese: "Bằng lái xe",
    image: "🚗",
  },

  {
    khmer: "បង្កាន់ដៃ",
    roman: "Boong-canh đay",
    vietnamese: "Biên nhận",
    image: "🧾",
  },

  {
    khmer: "កំណត់ហេតុ",
    roman: "Com-noth het",
    vietnamese: "Biên bản",
    image: "📋",
  },

  {
    khmer: "ធ្វើកំណត់ហេតុ",
    roman: "Th'vơ Com-noth het",
    vietnamese: "Lập biên bản",
    image: "📝",
  },

  {
    khmer: "លិខិតសន្យា",
    roman: "Lí-khet sonh-na-da",
    vietnamese: "Giấy cam đoan / Cam kết",
    image: "📜",
  },

  {
    khmer: "ធ្វើកិច្ចសន្យា / ធ្វើលិខិតសន្យា",
    roman: "Th'vơ kêch Son-na-da / Th'vơ Lí-khet Son-na-da",
    vietnamese: "Làm cam đoan / Cam kết",
    image: "✍️",
  },

  {
    khmer: "ចុះហត្ថលេខា / សរសេរឈ្មោះ",
    roman: "Chôs hăth-thă-lê-kha / Si-nhê chh'muas",
    vietnamese: "Ký tên",
    image: "✍️",
  },

  {
    khmer: "សំបុត្រអាពាហ៍ពិពាហ៍",
    roman: "Som-bôt a-pia-pi-pia",
    vietnamese: "Giấy đăng ký kết hôn",
    image: "💍",
  },

  /* =======================================================
     IV. LIÊN HỆ – CÔNG VIỆC – GIAO TIẾP
  ======================================================= */

  {
    khmer: "ទូរស័ព្ទ",
    roman: "Tu-ra-săp",
    vietnamese: "Điện thoại",
    image: "📱",
  },

  {
    khmer: "លេខទូរស័ព្ទ",
    roman: "Lêk Tu-ra-săp",
    vietnamese: "Số điện thoại",
    image: "📱",
  },

  {
    khmer: "សុំលេខទូរស័ព្ទ",
    roman: "Sôm lêk Tu-ra-săp",
    vietnamese: "Xin số điện thoại",
    image: "📱",
  },

  {
    khmer: "ហៅទូរស័ព្ទ",
    roman: "Hau (tê) Tu-ra-sâp",
    vietnamese: "Gọi điện thoại",
    image: "📞",
  },

  {
    khmer: "ទំនាក់ទំនង",
    roman: "Tôm-nek tôm-nôông",
    vietnamese: "Liên hệ",
    image: "🤝",
  },

  {
    khmer: "មានទំនាក់ទំនង",
    roman: "Mian tôm-nek tôm-nôông",
    vietnamese: "Có mối quan hệ",
    image: "🤝",
  },

  {
    khmer: "ទំនាក់ទំនងតាមលេខទូរស័ព្ទ",
    roman: "Tôm-nek tôm-nôông tam lêk Tu-ra-săp",
    vietnamese: "Liên hệ qua số điện thoại",
    image: "📱",
  },

  {
    khmer: "ណាត់",
    roman: "Nach",
    vietnamese: "Hẹn",
    image: "📅",
  },

  {
    khmer: "បញ្ហា",
    roman: "Panh-ha",
    vietnamese: "Vấn đề",
    image: "⚠️",
    note: '"Panh" đọc giọng Bắc.',
  },

  {
    khmer: "រឿង",
    roman: "Rương",
    vietnamese: "Chuyện",
    image: "💬",
  },

  {
    khmer: "ការ / កិច្ចការ",
    roman: "Ca / Ca nghia / Kêch-ca",
    vietnamese: "Việc / Công việc",
    image: "💼",
  },

  {
    khmer: "ពិបាក",
    roman: "Pi-bak",
    vietnamese: "Khó / Khó khăn",
    image: "😓",
  },

  {
    khmer: "ងាយ / ងាយស្រួល",
    roman: "Nghiay / Nghiay s'rual",
    vietnamese: "Dễ / Dễ dàng",
    image: "😊",
  },

  {
    khmer: "មានអី?",
    roman: "Mian ây?",
    vietnamese: "Có gì?",
    image: "❓",
  },

  {
    khmer: "មានបញ្ហាអី...?",
    roman: "Mian panh-ha ây...?",
    vietnamese: "Có vấn đề gì...?",
    image: "⚠️",
  },

  {
    khmer: "មានរឿងអី...?",
    roman: "Mian rương ây...?",
    vietnamese: "Có chuyện gì...?",
    image: "💬",
  },

  {
    khmer: "មានការអី...?",
    roman: "Mian ca ây...?",
    vietnamese: "Có việc gì / Có công việc gì...?",
    image: "💼",
  },

  {
    khmer: "មានអីពិបាក...?",
    roman: "Mian ây pi-bak...?",
    vietnamese: "Có gì khó khăn...?",
    image: "❓",
  },
];
/* =========================================================
   CHỦ ĐỀ 8: THỜI GIAN, NGÀY THÁNG & VỊ TRÍ
========================================================= */

const time = [

  /* -------------------------------------------------------
     1. ĐƠN VỊ & KHÁI NIỆM THỜI GIAN
  ------------------------------------------------------- */

  {
    khmer: "ពេលវេលា",
    roman: "Pêl vê-lia",
    vietnamese: "Thời gian",
    image: "⏰",
  },

  {
    khmer: "ម៉ោង",
    roman: "Môông",
    vietnamese: "Giờ",
    image: "🕐",
  },

  {
    khmer: "នាទី",
    roman: "Nia-ty",
    vietnamese: "Phút",
    image: "⏱️",
  },

  {
    khmer: "វិនាទី",
    roman: "Ví nia-ty",
    vietnamese: "Giây",
    image: "⏲️",
  },

  {
    khmer: "ពេល",
    roman: "Pêl",
    vietnamese: "Buổi",
    image: "🌅",
  },

  {
    khmer: "ថ្ងៃ",
    roman: "Th'ngay",
    vietnamese: "Ngày",
    image: "📅",
  },

  {
    khmer: "អាទិត្យ / សប្ដាហ៍",
    roman: "Atit / Săp-pa-đa",
    vietnamese: "Tuần",
    image: "🗓️",
  },

  {
    khmer: "ខែ",
    roman: "Khe",
    vietnamese: "Tháng",
    image: "📆",
  },

  {
    khmer: "ឆ្នាំ",
    roman: "Chh'năm",
    vietnamese: "Năm",
    image: "📅",
  },

  {
    khmer: "បច្ចុប្បន្ន",
    roman: "Pach-chôp-bonh",
    vietnamese: "Hiện tại",
    image: "⏳",
  },

  {
    khmer: "សព្វថ្ងៃ",
    roman: "Sop th'ngay",
    vietnamese: "Hiện nay / Hiện giờ",
    image: "🕒",
  },

  {
    khmer: "អតីតកាល",
    roman: "Ă-tây-tă-kal",
    vietnamese: "Quá khứ",
    image: "⏮️",
  },

  {
    khmer: "អនាគត",
    roman: "Ă-na-côth",
    vietnamese: "Tương lai",
    image: "⏭️",
  },


  /* -------------------------------------------------------
     2. CÁC BUỔI TRONG NGÀY & MỐC THỜI GIAN
  ------------------------------------------------------- */

  {
    khmer: "ព្រឹក",
    roman: "P'rức",
    vietnamese: "Sáng",
    image: "🌅",
  },

  {
    khmer: "ថ្ងៃត្រង់",
    roman: "Th'ngay t'roong / T'roong",
    vietnamese: "Trưa",
    image: "☀️",
  },

  {
    khmer: "រសៀល",
    roman: "Rô-siêl",
    vietnamese: "Xế chiều",
    image: "🌤️",
  },

  {
    khmer: "ល្ងាច",
    roman: "Lô-nghiach",
    vietnamese: "Chiều tối",
    image: "🌇",
  },

  {
    khmer: "យប់",
    roman: "Dốp",
    vietnamese: "Tối",
    image: "🌙",
  },

  {
    khmer: "ឥឡូវ",
    roman: "Ây-lâu",
    vietnamese: "Bây giờ",
    image: "⏰",
  },

  {
    khmer: "ម្សិលម្ថ្ងៃ",
    roman: "Ma-sel ma-ngay",
    vietnamese: "Hôm kia",
    image: "📅",
  },

  {
    khmer: "ម្សិលមិញ",
    roman: "Ma-sel minh / Ma-sel",
    vietnamese: "Hôm qua",
    image: "⬅️",
  },

  {
    khmer: "ថ្ងៃមុន",
    roman: "Th'ngay munh",
    vietnamese: "Hôm trước",
    image: "◀️",
  },

  {
    khmer: "ថ្ងៃនេះ",
    roman: "Th'ngay nis",
    vietnamese: "Hôm nay",
    image: "📅",
  },

  {
    khmer: "ថ្ងៃស្អែក",
    roman: "Th'ngay sa-ec / Sa-ec",
    vietnamese: "Ngày mai",
    image: "➡️",
  },

  {
    khmer: "ខាងស្អែក",
    roman: "Khane sa-ec",
    vietnamese: "Ngày mốt",
    image: "⏩",
  },

  {
    khmer: "រាល់ថ្ងៃ",
    roman: "Ról th'ngay",
    vietnamese: "Hàng ngày",
    image: "🔄",
  },


  /* -------------------------------------------------------
     3. CÁC NGÀY TRONG TUẦN
  ------------------------------------------------------- */

  {
    khmer: "ថ្ងៃច័ន្ទ",
    roman: "Th'ngay chanh",
    vietnamese: "Thứ 2",
    image: "2️⃣",
  },

  {
    khmer: "ថ្ងៃអង្គារ",
    roman: "Th'ngay oong-kia",
    vietnamese: "Thứ 3",
    image: "3️⃣",
  },

  {
    khmer: "ថ្ងៃពុធ",
    roman: "Th'ngay puth",
    vietnamese: "Thứ 4",
    image: "4️⃣",
  },

  {
    khmer: "ថ្ងៃព្រហស្បតិ៍",
    roman: "Th'ngay prô-hos",
    vietnamese: "Thứ 5",
    image: "5️⃣",
  },

  {
    khmer: "ថ្ងៃសុក្រ",
    roman: "Th'ngay sôk",
    vietnamese: "Thứ 6",
    image: "6️⃣",
  },

  {
    khmer: "ថ្ងៃសៅរ៍",
    roman: "Th'ngay sau",
    vietnamese: "Thứ 7",
    image: "7️⃣",
  },

  {
    khmer: "អាទិត្យ",
    roman: "A-tit",
    vietnamese: "Chủ nhật",
    image: "☀️",
  },


  /* -------------------------------------------------------
     4. CÁC THÁNG TRONG NĂM
  ------------------------------------------------------- */

  {
    khmer: "ខែមករា",
    roman: "Khe mă-ca-ra",
    vietnamese: "Tháng 1",
    image: "1️⃣",
  },

  {
    khmer: "ខែកុម្ភៈ",
    roman: "Khe côm-phek",
    vietnamese: "Tháng 2",
    image: "2️⃣",
  },

  {
    khmer: "ខែមីនា",
    roman: "Khe mí-nia",
    vietnamese: "Tháng 3",
    image: "3️⃣",
  },

  {
    khmer: "ខែមេសា",
    roman: "Khe mê-sa",
    vietnamese: "Tháng 4",
    image: "4️⃣",
  },

  {
    khmer: "ខែឧសភា",
    roman: "Khe uk-să-phia",
    vietnamese: "Tháng 5",
    image: "5️⃣",
  },

  {
    khmer: "ខែមិថុនា",
    roman: "Khe mí-thôk-na",
    vietnamese: "Tháng 6",
    image: "6️⃣",
  },

  {
    khmer: "ខែកក្កដា",
    roman: "Khe că-k'đa",
    vietnamese: "Tháng 7",
    image: "7️⃣",
  },

  {
    khmer: "ខែសីហា",
    roman: "Khe sây-ha",
    vietnamese: "Tháng 8",
    image: "8️⃣",
  },

  {
    khmer: "ខែកញ្ញា",
    roman: "Khe canh-nha",
    vietnamese: "Tháng 9",
    image: "9️⃣",
    note: '“Canh” đọc theo giọng Bắc.',
  },

  {
    khmer: "ខែតុលា",
    roman: "Khe tôk-la",
    vietnamese: "Tháng 10",
    image: "🔟",
  },

  {
    khmer: "ខែវិច្ឆិកា",
    roman: "Khe vích-chhek-ca",
    vietnamese: "Tháng 11",
    image: "1️⃣1️⃣",
  },

  {
    khmer: "ខែធ្នូ",
    roman: "Khe th'nu",
    vietnamese: "Tháng 12",
    image: "1️⃣2️⃣",
  },


  /* -------------------------------------------------------
     5. PHƯƠNG HƯỚNG & VỊ TRÍ
  ------------------------------------------------------- */

  {
    khmer: "មុន / មុខ",
    roman: "Munh / Muc",
    vietnamese: "Trước",
    image: "⬅️",
    note: "“Munh” dùng cho hành động; “Muc” dùng cho vị trí.",
  },

  {
    khmer: "ក្រោយ",
    roman: "Kroi",
    vietnamese: "Sau",
    image: "➡️",
  },

  {
    khmer: "នេះ",
    roman: "Nis",
    vietnamese: "Nay / Đây / Này",
    image: "👈",
  },

  {
    khmer: "នោះ",
    roman: "Nus",
    vietnamese: "Kia / Đó",
    image: "👉",
  },

  {
    khmer: "លើ",
    roman: "Lơ",
    vietnamese: "Trên",
    image: "⬆️",
  },

  {
    khmer: "ក្រោម",
    roman: "K'rom",
    vietnamese: "Dưới",
    image: "⬇️",
  },

  {
    khmer: "ក្នុង",
    roman: "Kh'ôông",
    vietnamese: "Trong",
    image: "🔲",
  },

  {
    khmer: "ក្រៅ",
    roman: "Krau",
    vietnamese: "Ngoài",
    image: "🔳",
  },

  {
    khmer: "ខាងណា",
    roman: "Khang na",
    vietnamese: "Phía nào",
    image: "🧭",
  },


  /* -------------------------------------------------------
     6. TỪ NGHI VẤN, TỪ NỐI & TRẠNG TỪ
  ------------------------------------------------------- */

  {
    khmer: "ពេលណា",
    roman: "Pêl na",
    vietnamese: "Khi nào",
    image: "❓",
  },

  {
    khmer: "ប៉ុន្មាន",
    roman: "Pôn-man",
    vietnamese: "Bao nhiêu",
    image: "🔢",
  },

  {
    khmer: "យ៉ាងម៉េច / យ៉ាងណា / ដូចម្តេច",
    roman: "Dang-mêch / Dang-na / Đôch m'đách",
    vietnamese: "Thế nào / Như thế nào",
    image: "🤔",
  },

  {
    khmer: "ហេតុអ្វី",
    roman: "Het ây",
    vietnamese: "Tại sao",
    image: "❓",
  },

  {
    khmer: "ម្ដងម្កាល",
    roman: "M'đoong m'kal",
    vietnamese: "Thỉnh thoảng",
    image: "🔄",
  },

  {
    khmer: "ជួនកាល",
    roman: "Chuône kal",
    vietnamese: "Đôi khi",
    image: "⏳",
  },

  {
    khmer: "ជាញឹកញាប់ / ញឹកញាប់",
    roman: "Chia nhưc nhop / Nhưc nhop",
    vietnamese: "Thường xuyên",
    image: "🔁",
  },

  {
    khmer: "ក្នុងពេល",
    roman: "Kh'nông pêl",
    vietnamese: "Trong lúc",
    image: "⏱️",
  },

  {
    khmer: "មិនដែល",
    roman: "Mưnh đel",
    vietnamese: "Chưa bao giờ / Chưa từng",
    image: "🚫",
  },

  {
    khmer: "ទោះបី",
    roman: "Tuas bây",
    vietnamese: "Mặc dù",
    image: "↔️",
  },

  {
    khmer: "ដូចនេះ / ដូចច្នេះ",
    roman: "Đôch nis / Đôch ch'nes",
    vietnamese: "Vì vậy / Do đó",
    image: "➡️",
  },

  {
    khmer: "តែ / ប៉ុន្តែ",
    roman: "Te / Pôn-te",
    vietnamese: "Nhưng / Nhưng mà",
    image: "↔️",
  },

  {
    khmer: "ប្រហែល",
    roman: "Pro-hel",
    vietnamese: "Khoảng",
    image: "≈",
  },

  {
    khmer: "យូរ",
    roman: "Du",
    vietnamese: "Lâu",
    image: "⌛",
  },

  {
    khmer: "យូរៗម្ដង",
    roman: "Du-du m'đoong",
    vietnamese: "Lâu lâu một lần",
    image: "🔄",
  },

  {
    khmer: "កុំអាល",
    roman: "Côm al",
    vietnamese: "Khoan / Khoan đã",
    image: "✋",
  },
];
/* =========================================================
   CHỦ ĐỀ 9: TIỀN BẠC, THANH TOÁN & SỐ THỨ TỰ
========================================================= */

const moneyAndOrder = [
  /* -------------------------------------------------------
     TIỀN & THANH TOÁN
  ------------------------------------------------------- */

  {
    khmer: "លុយ / កាក់ / ប្រាក់",
    roman: "Lui / căk / p'răk",
    vietnamese: "Tiền",
    image: "💰",
  },

  {
    khmer: "គិតលុយ",
    roman: "Kít lui",
    vietnamese: "Tính tiền",
    image: "🧮",
  },

  {
    khmer: "បង់លុយ",
    roman: "Boong lui",
    vietnamese: "Đóng tiền / Nộp tiền",
    image: "💵",
  },

  {
    khmer: "អាប់លុយ",
    roman: "Ăp lui",
    vietnamese: "Thối tiền",
    image: "💸",
  },

  {
    khmer: "ពិនិត្យលុយ",
    roman: "Pi-nit lui",
    vietnamese: "Kiểm tra tiền",
    image: "🔍",
  },

  {
    khmer: "រាប់លុយ",
    roman: "Rop lui",
    vietnamese: "Đếm tiền",
    image: "🧾",
  },

  {
    khmer: "ទុកលុយ",
    roman: "Tuc lui",
    vietnamese: "Cất tiền",
    image: "👛",
  },

  {
    khmer: "ខ្ចី",
    roman: "Kh'chây",
    vietnamese: "Mượn tiền",
    image: "🤲",
  },

  {
    khmer: "សងលុយ",
    roman: "Soong lui",
    vietnamese: "Trả tiền",
    image: "💳",
  },

  {
    khmer: "អត់(មាន)លុយ",
    roman: "Oth (mian) lui",
    vietnamese: "Không (có) tiền",
    image: "🚫",
  },

  {
    khmer: "អស់លុយ",
    roman: "Os lui",
    vietnamese: "Hết tiền",
    image: "😅",
  },

  {
    khmer: "បោះត្រា",
    roman: "Bos-tra",
    vietnamese: "Đóng dấu",
    image: "🔖",
  },

  {
    khmer: "ផ្ដិតខ្ចៅដៃ",
    roman: "Ph'đet kh'chau đay",
    vietnamese: "Lăn tay",
    image: "☝️",
  },

  {
    khmer: "ស៊ីញ៉េឈ្មោះ / ចុះហត្ថលេខា",
    roman: "Si-nhê ch'muas / Chôs hăth-tha-lê-kha",
    vietnamese: "Ký tên",
    image: "✍️",
    note: "Có thể dùng Si-nhê ch'muas hoặc Chôs hăth-tha-lê-kha.",
  },

  {
    khmer: "គណនី",
    roman: "Kek-nă-nây",
    vietnamese: "Tài khoản",
    image: "👤",
  },

  {
    khmer: "ផ្ទេរលុយ",
    roman: "Ph'tê lui / bank lui",
    vietnamese: "Chuyển khoản",
    image: "🏦",
  },

  {
    khmer: "ធនាគារ",
    roman: "Thô-nia-kia",
    vietnamese: "Ngân hàng",
    image: "🏦",
  },

  {
    khmer: "ម៉ឺនុយ",
    roman: "Mê-nui",
    vietnamese: "Menu",
    image: "📋",
  },

  {
    khmer: "វិក្កយបត្រ",
    roman: "Vi-cay-dă-băth",
    vietnamese: "Hóa đơn",
    image: "🧾",
  },

  {
    khmer: "បង្កាន់ដៃ",
    roman: "Boong canh đay",
    vietnamese: "Biên nhận",
    image: "📄",
    note: 'Từ "Boong" đọc ngắn.',
  },

  {
    khmer: "បង្កាន់ដៃបង់លុយ",
    roman: "Boong canh đay boong lui",
    vietnamese: "Biên lai / Phiếu thu tiền",
    image: "🧾",
    note: 'Từ "Boong" đọc ngắn.',
  },

  {
    khmer: "ប័ណ្ណទូទាត់",
    roman: "Banh tu toth",
    vietnamese: "Phiếu thanh toán",
    image: "💳",
  },


  /* -------------------------------------------------------
     SỐ THỨ TỰ
     QUY TẮC: Ti + số đếm
  ------------------------------------------------------- */

  {
    khmer: "ទីមួយ",
    roman: "Ti muôi",
    vietnamese: "Thứ nhất (số thứ tự)",
    image: "🥇",
    note: "Ti + Muôi (một).",
  },

  {
    khmer: "ទីពីរ",
    roman: "Ti Pi",
    vietnamese: "Thứ hai (số thứ tự)",
    image: "🥈",
    note: "Ti + Pi (hai).",
  },

  {
    khmer: "ទីបី",
    roman: "Ti Bây",
    vietnamese: "Thứ ba (số thứ tự)",
    image: "🥉",
    note: "Ti + Bây (ba).",
  },

  {
    khmer: "ទីបួន",
    roman: "Ti Buôn",
    vietnamese: "Thứ tư (số thứ tự)",
    image: "4️⃣",
    note: "Ti + Buôn (bốn).",
  },

  {
    khmer: "ទីប្រាំ",
    roman: "Ti Prăm",
    vietnamese: "Thứ năm (số thứ tự)",
    image: "5️⃣",
    note: "Ti + Prăm (năm).",
  },

  {
    khmer: "ទីប្រាំមួយ",
    roman: "Ti Prăm - Muôi",
    vietnamese: "Thứ sáu (số thứ tự)",
    image: "6️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីប្រាំពីរ",
    roman: "Ti Prăm - Pi",
    vietnamese: "Thứ bảy (số thứ tự)",
    image: "7️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីប្រាំបី",
    roman: "Ti Prăm - Bây",
    vietnamese: "Thứ tám (số thứ tự)",
    image: "8️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីប្រាំបួន",
    roman: "Ti Prăm - Buôn",
    vietnamese: "Thứ chín (số thứ tự)",
    image: "9️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីដប់",
    roman: "Ti Đóp",
    vietnamese: "Thứ mười (số thứ tự)",
    image: "🔟",
    note: "Ti + Đóp (mười).",
  },

  {
    khmer: "ទីដប់មួយ",
    roman: "Ti Đóp - Muôi",
    vietnamese: "Thứ mười một (số thứ tự)",
    image: "1️⃣1️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីដប់ពីរ",
    roman: "Ti Đóp - Pi",
    vietnamese: "Thứ mười hai (số thứ tự)",
    image: "1️⃣2️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីដប់បី",
    roman: "Ti Đóp - Bây",
    vietnamese: "Thứ mười ba (số thứ tự)",
    image: "1️⃣3️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីដប់បួន",
    roman: "Ti Đóp - Buôn",
    vietnamese: "Thứ mười bốn (số thứ tự)",
    image: "1️⃣4️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីដប់ប្រាំ",
    roman: "Ti Đóp - Prăm",
    vietnamese: "Thứ mười lăm (số thứ tự)",
    image: "1️⃣5️⃣",
    note: "Ti + số đếm.",
  },

  {
    khmer: "ទីម្ភៃ",
    roman: "Ti Mô-phây",
    vietnamese: "Thứ hai mươi (số thứ tự)",
    image: "2️⃣0️⃣",
    note: "Ti + Mô-phây (hai mươi).",
  },
];
/* =========================================================
   DỮ LIỆU CHỦ ĐỀ
========================================================= */

const vocabularyCategories = [
  {
    id: "numbers",
    icon: "🔢",
    title: "Số đếm",
    description: "Số đếm và cách đọc các số lớn",
  },

  {
    id: "pronouns",
    icon: "👤",
    title: "Từ xưng hô",
    description: "Các từ xưng hô cơ bản trong tiếng Khmer",
  },

  {
    id: "communication",
    icon: "💬",
    title: "Giao tiếp",
    description: "Những từ và câu giao tiếp thông dụng hằng ngày",
  },

  {
    id: "family",
    icon: "👨‍👩‍👧‍👦",
    title: "Gia đình & quan hệ",
    description: "Gia đình, họ hàng và các mối quan hệ",
  },
  {
    id: "jobs",
    icon: "💼",
    title: "Nghề nghiệp & Chức vụ",
    description: "Các nghề nghiệp và chức vụ thường gặp",
  },
  {
    id: "body",
    icon: "🧍",
    title: "Cơ thể người",
    description: "Các bộ phận cơ thể và cơ quan bên trong",
  },
  {
    id: "addressAdmin",
    icon: "📍",
    title: "Địa chỉ – Giấy tờ – Liên hệ",
    description: "Địa chỉ, hành chính, giấy tờ và giao tiếp liên hệ",
  },
  {
  id: "time",
  icon: "⏰",
  title: "Thời gian",
  description: "Thời gian, ngày tháng, vị trí và các từ chỉ thời gian",
  },
  {
  id: "moneyAndOrder",
  icon: "💰",
  title: "Tiền bạc & Số thứ tự",
  description: "Tiền bạc, thanh toán và cách nói số thứ tự",
  },
];

const vocabularyData = {
  numbers,
  pronouns,
  communication,
  family,
  jobs,
  body,
  addressAdmin,
  time,
  moneyAndOrder,
};

export {
  vocabularyCategories,
  vocabularyData,
};

export default vocabularyData;